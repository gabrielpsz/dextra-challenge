...
@Slf4j
@RestController
@RequestMapping("/v2/public/characters")
public class ReactiveCharacterController {

    private CharacterService characterService;

    public ReactiveCharacterController(CharacterService characterService) {
        this.characterService = characterService;
    }

    @GetMapping
    public ResponseEntity<List<MarvelCharacter>> getAllCharacters() throws ApiErrorException {
        log.info("Request to get all characters");
        return ResponseEntity.ok().body(characterService.getAllCharacters());
    }

    @GetMapping("/{id}")
    public ResponseEntity<MarvelCharacter> getCharacterById(@PathVariable String id) throws Exception {
        log.info("Request to get character with id {}", id);
        return ResponseEntity.ok().body(characterService.getCharacterById(id));
    }

    @GetMapping("/{id}/comics")
    public ResponseEntity<List<Comics>> getCharacterComics(@PathVariable String id) throws Exception {
